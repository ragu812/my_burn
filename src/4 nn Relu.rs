...
use burn::nn::{Linear,LinearConfig,Relu,Initializer};
...
fn main(){
   ...
   let signs_tensor = Tensor::<Backend,2>::from_data(
    ...
    let final_output_scalar: f32 = final_output.clone().into_scalar();
    let pi_value_scalar: f32 = pi_value.clone().into_scalar();
